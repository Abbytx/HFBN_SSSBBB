...
=======
    train_loader = DataLoader(DataLoadAdni(partition='train', num_points=args.num_points, partroi=args.partroi, adni=args.adni, splitt=args.splitt, fold=fold, choose_data=args.data_choose), num_workers=0,
                            batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(DataLoadAdni(partition='test', num_points=args.num_points, partroi=args.partroi, adni=args.adni, splitt=args.splitt, fold=fold, choose_data=args.data_choose), num_workers=0,
>>>>>>> parent of 4884111 (hierarchical-model):main.py
...
>>>>>>> parent of 4884111 (hierarchical-model):main.py
    ...
    parser.add_argument('--partroi', type=int, default=270)#,choices=[42,90,100,901]
    parser.add_argument('--splitt', type=int, default=2, choices=[0,1,2,3,4,5,6])
    ...
    parser.add_argument('--hidden_dim', type=int, default=90, help=' ')
...
=======
    parser.add_argument('--sparse_alpha', type=float, default=1.5, help=' ')
    parser.add_argument('--data_choose', type=str, default=None, help='choose model')
    parser.add_argument('--fold_list',  default=[0,1,2,3,4], help='')
>>>>>>> parent of 4884111 (hierarchical-model):main.py
    allaccu = []
    all_result = []
...
=======
        args.model_path = str(args.log_dir)+'/'+'adni'+str(args.adni)+'_roi'+str(args.partroi)+'_pool'+str(args.num_pooling)+'_fold'+str(i+1)+'/train/models/model.t7'
>>>>>>> parent of 4884111 (hierarchical-model):main.py
        _init_()
        io = IOStream('log/' + args.exp_name + '/run.log')
        io.cprint(str(args))
...
=======
    with open('log/adni_{}_roi_{}_log.txt'.format(args.adni,args.partroi),'a') as f:
        print('*************:', file=f)
        print( 'adni' + str(args.adni) + '_roi' + str(args.partroi)  + '_lr' + str(args.lr) + '_BS' + str(args.batch_size) + '_pool' + str(args.num_pooling) + '_rate' + str(
            args.assign_ratio) +':', file=f)
>>>>>>> parent of 4884111 (hierarchical-model):main.py
        print(array, file=f)
